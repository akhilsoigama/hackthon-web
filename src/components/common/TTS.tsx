...
const MultiLanguageTTS: React.FC<MultiLanguageTTSProps> = ({ 
  ...
  const handleSpeak = async (): Promise<void> => {
    const langConfig = availableLanguages.find(l => l.code === selectedLanguage);
    ...
    if (langConfig.voiceSupport) {
      ...
      if (translation) {
        ...
        toast.info(`Voice not available for ${langConfig.name}. Translation: ${translation}`);
      }
    }
  };
  ...
  if (compact) {
    return (
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {availableLanguages.filter(lang => lang.voiceSupport).length > 4 && (
          <select 
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
        ...
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex justify-between items-start">
            <h4 className="font-medium text-blue-800 mb-2">
              Translation in {availableLanguages.find(l => l.code === selectedLanguage)?.name}:
            </h4>
            <button
              onClick={() => handleQuickSpeak(selectedLanguage)}
              disabled={isSpeaking || isTranslating || !availableLanguages.find(l => l.code === selectedLanguage)?.voiceSupport}
      ...
      <div className="mt-4">
        ...
        <div className="mb-3">
          <h5 className="text-sm font-medium text-green-700 mb-2">🔊 With Voice Support:</h5>
              ...
              .map((lang) => (
                ...
                >
                  <div className="flex justify-between items-center">
                    <span className="font-medium text-green-900">{lang.name}</span>
                    <span className="text-xs bg-green-200 text-green-800 px-1 rounded">🔊</span>
                  </div>
        ...
        {showTranslations && (
          <div>
            <h5 className="text-sm font-medium text-yellow-700 mb-2">📝 Translation Only:</h5>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              {availableLanguages
                .filter(lang => !lang.voiceSupport)
                .map((lang) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-medium text-yellow-900">{lang.name}</span>
                      <span className="text-xs bg-yellow-200 text-yellow-800 px-1 rounded">📝</span>
                    </div>
                    <div className="text-xs text-yellow-700">{lang.nativeName}</div>
      ...
      <div className="text-sm text-gray-600 mt-3">
        {isSpeaking && (
          <div className="flex items-center text-green-600">
            ...
            Speaking in {availableLanguages.find(l => l.code === currentLanguage)?.name || 'English'}
          </div>
        )}
        {isTranslating && (
